...
某物流中心有2 台配送车辆,其载重量均为8t ,车辆每次配送的最大行驶距离为50km ,
配送中心(其编号为0) 与8 个客户之间及8 个客户相互之间的距离dij 、8 个客户的货物需求量qj (i 、j = 1 ,2 , ⋯,8)
要求合理安排车辆配送路线,使配送总里程最短。

采用以下参数:
群体规模取20 ,进化代数取25 ,交叉概率取0.9 ,变异概率取0.09 ,变异时基因换位次数取5 ,
对不可行路径的惩罚权重取100km ,实施爬山操作时爬山次数取20 。对实例随机求解10 次。
...
if __name__ == '__main__':
    ga = GeneticAlgorithm(rows=20, times=25, mans=12, cars=8, tons=4100, distance=100000, PW=100)
